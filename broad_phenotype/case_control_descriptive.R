...
ggsave(paste0("data/broad_pheno_EUR_case_control_",cat_var,"_plot.png"))
}

df <- demo %>% select(broad_pheno_1_5_ratio,genetic_sex)
df$genetic_sex <- as.factor(df$genetic_sex)
categorical_plot(df,as.character(colnames(df)[2]),"Severe Asthma")

df <- demo %>% select(broad_pheno_1_5_ratio,sex)
df$sex <- as.factor(df$sex)
categorical_plot(df,as.character(colnames(df)[2]),"Severe Asthma")

df <- demo %>% select(broad_pheno_1_5_ratio,sex_sample_file)
df$sex_sample_file <- as.factor(df$sex_sample_file)
categorical_plot(df,as.character(colnames(df)[2]),"Severe Asthma")

df <- demo %>% select(broad_pheno_1_5_ratio,category_onset)
categorical_plot(df,as.character(colnames(df)[2]),"Severe Asthma")

df <- demo %>% select(broad_pheno_1_5_ratio,smoking_status)
...
df %>%
  ...
  axis.title.y = element_text(size = 16), axis.title.x = element_text(size = 16),
  legend.title = element_text(size = 18), legend.text = element_text(size = 17))
ggsave(paste0("data/broad_pheno_EUR_case_control_",as.character(colnames(df)[2]),"_plot.png"))

df <- demo %>% select(broad_pheno_1_5_ratio,clustered_ancestry)
...
df <- demo_hesin %>% select(broad_pheno_1_5_ratio,hesin)
...
ggsave(paste0("data/broad_pheno_EUR_case_control_",num_var,"_plot.png"))
}

df <- demo %>% select(broad_pheno_1_5_ratio, perc_pred_FEV1)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo %>% select(broad_pheno_1_5_ratio, ratio_FEV1_FVC)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo %>% select(broad_pheno_1_5_ratio,best_FEV1)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo %>% select(broad_pheno_1_5_ratio,BMI)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo %>% select(broad_pheno_1_5_ratio,age_at_recruitment)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo %>% select(broad_pheno_1_5_ratio,cigarette_pack_years)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")
...
df <- demo_eos %>% select(broad_pheno_1_5_ratio,max_eos)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")
...
df <- demo_neu %>% select(broad_pheno_1_5_ratio,max_neu)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")
...
df <- demo %>% select(broad_pheno_1_5_ratio,ubiopred_smk)
...
#case-control:
print("Cases-control broad phenotype")
summary(demo$broad_pheno_1_5_ratio)
prop.table(table(demo$broad_pheno_1_5_ratio))

cases <- demo %>% filter(broad_pheno_1_5_ratio == 1)
controls <- demo %>% filter(broad_pheno_1_5_ratio == 0)

print("Summary statistics broad phenotype - cases and controls")
...
demo_eos_cases <- demo_eos %>% filter(broad_pheno_1_5_ratio == 1)
...
demo_eos_controls <- demo_eos %>% filter(broad_pheno_1_5_ratio == 0)
...
demo_neu_cases <- demo_neu %>% filter(broad_pheno_1_5_ratio == 1)
...
demo_neu_controls <- demo_neu %>% filter(broad_pheno_1_5_ratio == 0)
...
demo_hesin_cases <- left_join(demo,hesin,by="eid") %>% filter(broad_pheno_1_5_ratio == 1)
print("Cases")
table(demo_hesin_cases$hesin,exclude=NULL)
prop.table(table(demo_hesin_cases$hesin,exclude=NULL))
print("Controls")
demo_hesin_controls <- left_join(demo,hesin,by="eid") %>% filter(broad_pheno_1_5_ratio == 0)
...
df <- demo %>% filter(broad_pheno_1_5_ratio == 1) %>% select(pred_use, perc_pred_FEV1) %>% rename(perc_pred_FEV1_by_pred_use=perc_pred_FEV1)
...
df <- demo %>% filter(broad_pheno_1_5_ratio == 1) %>% select(pred_use, ratio_FEV1_FVC) %>% rename(ratio_FEV1_FVC_by_pred_use=ratio_FEV1_FVC)
df$pred_use <- as.factor(df$pred_use)
numeric_plot(df, as.character(colnames(df)[2]),"Prednisolone use")

#Best_FEV1 by prednisolone use:
df <- demo %>% filter(broad_pheno_1_5_ratio == 1) %>% select(pred_use, best_FEV1) %>% rename(best_FEV1_by_pred_use=best_FEV1)
df$pred_use <- as.factor(df$pred_use)
numeric_plot(df, as.character(colnames(df)[2]),"Prednisolone use")

#hesin by prednisolone use:
demo_hesin <- left_join(demo,hesin,by="eid")
df <- demo_hesin %>% filter(broad_pheno_1_5_ratio == 1) %>% select(pred_use, hesin) %>% rename(hesin_by_pred_use=hesin)
df$pred_use <- as.factor(df$pred_use)
...
cases <- demo_LF_kath %>% filter(broad_pheno_1_5_ratio == 1)
controls <- demo_LF_kath %>% filter(broad_pheno_1_5_ratio == 0)
...
df <- demo_LF_kath %>% select(broad_pheno_1_5_ratio, fev1_perc_pred)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")

df <- demo_LF_kath %>% select(broad_pheno_1_5_ratio, ff.best)
numeric_plot(df, as.character(colnames(df)[2]),"Difficult-to-treat")
